...
void mover_parallel(Points *particle_list, double step_x, double step_y) {
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < NUM_Points; i++) {
            double drift = cos(particle_list[i].y) * 0.000015; 
            particle_list[i].x += (double)rand_r(&seed) / RAND_MAX * 0.02 - 0.01;
            particle_list[i].y += (double)rand_r(&seed) / RAND_MAX * 0.02 - 0.01 + drift;
    ...
    #pragma omp parallel
    {
        unsigned int seed = (unsigned int)time(NULL) ^ (omp_get_thread_num() + 5);
        #pragma omp for
        for (int i = 0; i < NUM_Points; i++) {
